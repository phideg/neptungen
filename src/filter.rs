...
pub fn is_image(entry: &DirEntry) -> bool {
    entry
        ...
        .to_str()
        .map(|f| {
            f.ends_with(".jpg") || f.ends_with(".JPG") || f.ends_with(".png") ||
                f.ends_with(".PNG") || f.ends_with(".gif") || f.ends_with(".GIF")
        })
        .unwrap_or(false)
}

pub fn contains_markdown_file(entry: &DirEntry) -> bool {
    WalkDir::new(entry.path()).into_iter().any(|e| {
        e.is_ok() && is_markdown(e.as_ref().unwrap())
    })
}

pub fn contains_markdown_subdir(entry: &DirEntry) -> bool {
    WalkDir::new(entry.path()).min_depth(1).into_iter().any(
        |e| {
